...
function buildCardHtml(
  ...
  const position =
    POS[member.position ?? "Athlete"] ?? member.position ?? "Athlete";
  const cat = CAT[member.category] ?? member.category;
  const parts = member.fullName.trim().split(" ");
  const first = parts[0] ?? "";
  const last = parts.slice(1).join(" ");
  const firstAr = member.firstNameAr ?? "";
  const lastAr = member.lastNameAr ?? "";
...
.name{position: absolute;
    ...
    z-index: 5;
    white-space: nowrap;}
.nf{font:700 italic 37px/1 'Cormorant Garamond',serif;color:#2a6b2a}
.nl{font:400 italic 37px/1 'Cormorant Garamond',serif;color:#7a0000;margin-left:10px}
...
.fields{position:absolute;top:297px;left:36px;z-index:5;display:flex;flex-direction:column;gap:15px}
...
.flb{font:400 20px/1 'Inter',sans-serif;color:#7a4a20;min-width:180px}
.fd{font:300 14px/1 'Inter',sans-serif;color:#c8a060;margin:0 3px}
.fv{font:600 17px/1 'Inter',sans-serif;color:#3d0000}
.lic{position: absolute;
    top: 281px;
    right: 48px;
    z-index: 5;
    width: 214px;}
.lic-hd{font: 520 15px / 1 'Inter', sans-serif;
    letter-spacing: 2.5px;
    text-transform: uppercase;
    color: #7a4a20;
    margin-bottom: 8px;
    margin-left: 6px;}
.lic-box{border: 1.5px solid rgba(139, 0, 0, 0.22);
    border-radius: 14px;
    overflow: hidden;
    background: #fff;
...
.lic-v{font:400 13px/1 'Inter',sans-serif;color:#aaa;margin-top:8px}
...
.bmag{display:none; position:absolute;top:96px;left:0;right:0;height:44px;background:#040804;z-index:4}
...
<div class="card front">
  ...
  <div class="name">
    ...
    <div className="nameAr">
      <span class="nf">${firstAr}</span><span class="nl">${lastAr ? "&nbsp;" + lastAr : ""}</span>
    </div>

  </div>
  <div class="rule"><div class="rd"></div><div class="rl"></div></div>
        <div class="lic-s season2">Saison ${card.season}</div> 
  ...
  <div class="field">
    ...
    <div class="field">
      ...
      <span class="flb">Genre</span><span class="fd">—</span><span class="fv">${gender}</span>
       ...
       <div class="type_cnt">
             <span class="type_text"> ${position}</span>
      </div>
  ...
  <div class="qr"><img src="${backQrUrl}" alt="QR"/></div>
...
<div class="card back">
  ...
  <!-- <div class="bgrid">
    ...
    <div class="backText">
        <p class="backTextP"> Cette carte est strictement personnelle et réservée aux affiliés de la Fédération Royale Marocaine d’Aviron.  </p>
        <p class="backTextP"> Toute utilisation frauduleuse ou cession à des tiers est passible de sanctions conformément au règlement intérieur de la fédération. </p>   
